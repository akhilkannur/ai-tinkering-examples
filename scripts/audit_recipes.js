...
const LOG_FILE = path.join(__dirname, '../docs/RECIPE_AUDIT_LOG.md');
...
function getIgnoredFiles() {
  ...
  // Match `filename` inside backticks or just lines starting with * ✅
  const matches = content.match(/`([^`]+)`/g) || [];
  return matches.map(m => m.replace(/`/g, '') + '.md');
}

function auditRecipes() {
  const allFiles = fs.readdirSync(RECIPES_DIR).filter(f => f.endsWith('.md'));
  const ignoredFiles = getIgnoredFiles();
  const filesToAudit = allFiles.filter(f => !ignoredFiles.includes(f));

  let slopCount = 0;
  let strongCount = 0;
  let auditLog = `# Full Recipe Audit Report (Excluding Previously Upgraded)\n\nGenerated on: ${new Date().toLocaleDateString()}\n\n`;
  ...
  auditLog += `## Summary\n`;
  auditLog += `- **Total Files Scanned:** ${filesToAudit.length} (Excluded ${ignoredFiles.length} already audited)\n`;
  auditLog += `- **🔴 Likely Slop (Needs Upgrade):** ${slopCount}\n`;
  auditLog += `- **🟢 Likely Strong (Has Research/Logic):** ${strongCount}\n`;
  auditLog += `- **⚪ Neutral:** ${filesToAudit.length - slopCount - strongCount}\n\n`;
