...
async function enrichAndCapture(url, slug) {
  ...
  try {
    ...
    try {
        ...
        console.log(`⚠️ Navigation timeout for ${url}, attempting to scrape current state...`);
    }
...
async function runBatch() {
  const allTools = JSON.parse(fs.readFileSync('final_unique_queue.json', 'utf8'));
  const results = [];
  ...
  fs.writeFileSync('final_enrichment_results.json', JSON.stringify(results, null, 2));
  console.log(`\n🚀 Final enrichment complete. Results saved.`);
}
