...
export default function ExampleCard({ example, sponsor, priority = false, onOpen }: ExampleCardProps) {
  ...
  return (
    <motion.article
      ...
      className="card group cursor-pointer relative flex flex-col bg-secondary-bg shadow-lg rounded-2xl overflow-hidden border border-transparent hover:border-accent transition-all duration-300 transform hover:-translate-y-1"
    >
      ...
      <div className="relative z-10 flex-grow" onClick={handleCardClick}>
        {img && (
          <div className="relative w-full h-64 overflow-hidden rounded-t-2xl bg-slate-100 shadow-lg"> 
            ...
            {/* Title Overlay */}
            <div className="absolute bottom-0 left-0 right-0 p-4 bg-primary-bg/70 rounded-b-2xl"> 
              <h3 className="text-lg font-semibold leading-tight text-text-color line-clamp-2">
                {example.title}
              </h3>
            </div>
          </div>
