  ...
  return (
    <div>
      ...
      <div>
        ...
        {/* Billboard Ad (Premium Coffee/Pine Teal theme) */}
        {billboardAd && (
          <div className="max-w-5xl mx-auto mb-16 md:mb-28 px-6">
            <a 
              ...
              className="group block relative overflow-hidden rounded-sm border border-white/5 bg-coffee-900 backdrop-blur-md p-10 md:p-16 transition-all hover:border-[#064e3b]/30 shadow-2xl"
            >
              ...
              <div className="flex flex-col md:flex-row items-center gap-10 md:gap-16 relative z-10">
                {billboardAd.logo ? (
                  ...
                  <div className="w-24 h-24 md:w-32 md:h-32 bg-white/5 border border-white/10 rounded-sm flex items-center justify-center flex-shrink-0">
                    <Megaphone className="w-10 h-10 md:w-16 md:h-16 text-[#064e3b] animate-pulse" />
                  </div>
                )}
                <div className="text-center md:text-left flex-1">
                  <h3 className="text-3xl md:text-5xl font-bold text-white mb-6 group-hover:text-[#064e3b] transition-colors tracking-tight leading-[1.1]">{billboardAd.title}</h3>
              ...
              <div className="absolute bottom-0 right-0 w-96 h-96 bg-[#064e3b]/10 rounded-full blur-[120px] -mb-48 -mr-48"></div>
            </a>
          </div>
        )}
                                ...
                                return (
                                  <React.Fragment key={tool.name}>
                                    <Link href={`/tools/${slugify(tool.name)}`}>
                                      <div className={`${isFeaturedInDrop ? 'bg-[#f0fdf4] border-l-4 border-l-[#064e3b] shadow-inner-soft' : ''}`}>
                                        <ToolDataRow 
                                          tool={tool} 
                                          isDirectory={true} 
                                      ...
                                      >
                                        <div className="group flex flex-col md:flex-row md:items-center gap-6 py-12 px-8 bg-coffee-50/50 border border-coffee-200 border-dashed hover:border-[#064e3b]/40 transition-all cursor-pointer rounded-sm mb-4 relative overflow-hidden shadow-sm">
                                          ...
                                          <div className="flex-1 min-w-0">
                                            <p className="text-[15px] text-coffee-600/60 font-medium italic leading-relaxed text-balance">Pin your tool to the top of this week's drop for maximum community visibility.</p>
                                          </div>
  ...
  return (
    <a 
      ...
      className={`group relative flex flex-col p-12 rounded-sm border transition-all ${
        ...
        : 'bg-[#f0fdf4] border-[#064e3b]/20 shadow-sm hover:shadow-2xl hover:border-[#064e3b]/40'
      }`}
    >
      {!ad.isPlaceholder && (
...
function InlineAdBanner({ ad }: { ad: AdSpot }) {
  return (
    ...
    >
      ...
      <div className={`px-10 py-5 rounded-sm font-black uppercase tracking-widest text-[11px] transition-all whitespace-nowrap shadow-xl ${
        ad.isPlaceholder
...
function ToolTile({ tool, isFeatured }: { tool: AiTool, isFeatured?: boolean }) {
  ...
  return (
    <div className={`group flex flex-col h-full rounded-sm p-8 transition-all cursor-pointer border ${isFeatured ? 'bg-[#f0fdf4] border-[#064e3b]/30 shadow-xl z-10' : 'bg-white border-micro-layer-1 hover:border-micro-fg hover:shadow-micro'}`}>
      <div className="flex items-center gap-5 mb-6 relative">
        ...
        <div className="min-w-0 flex-1">
          <h3 className={`text-lg font-bold tracking-tight group-hover:underline decoration-2 underline-offset-4 truncate ${isFeatured ? 'text-[#064e3b]' : 'text-micro-fg'}`}>
            {tool.name}
          </h3>
      ...
      <p className={`text-[15px] font-medium leading-relaxed line-clamp-2 flex-1 mb-6 ${isFeatured ? 'text-[#064e3b]/80' : 'text-micro-muted'}`}>
        {tool.description}
      </p>
...
function ToolDataRow({ tool, isDirectory, isFeatured }: { tool: AiTool, isDirectory?: boolean, isFeatured?: boolean }) {
  ...
  return (
    <div
      className={`group flex flex-col md:flex-row md:items-center gap-8 py-10 px-8 -mx-8 transition-all cursor-pointer rounded-sm border border-transparent ${isFeatured ? 'bg-[#f0fdf4]/50' : 'hover:bg-white hover:border-micro-layer-1 hover:shadow-soft'}`}
    >
      ...
      <div className="flex items-center gap-8 flex-shrink-0 md:w-72 relative">
        ...
        <div className="min-w-0">
          <h3 className={`text-xl font-bold tracking-tight group-hover:underline decoration-2 underline-offset-4 ${isFeatured ? 'text-[#064e3b]' : 'text-micro-fg'}`}>
            {tool.name}
          </h3>
          <span className={`text-[11px] font-bold uppercase tracking-[0.1em] ${isFeatured ? 'text-[#064e3b]/60' : 'text-micro-muted'}`}>
            {tool.category}
      ...
      {/* Description */}
      <div className="flex-1 min-w-0">
        <p className={`text-[17px] font-medium leading-relaxed ${isDirectory ? 'truncate' : ''} ${isFeatured ? 'text-[#064e3b]/80' : 'text-micro-muted'}`}>
          {tool.description}
        </p>
      </div>
