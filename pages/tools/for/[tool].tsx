...
export default function ToolLandingPage({ toolName, toolSlug, recipes, categories }: ToolLandingPageProps) {
  ...
  const title = `Best ${config.name} Prompts & Workflows for Work (2026)`;
  const description = `Stop chatting, start building. ${recipes.length} field-tested blueprints optimized for ${config.name} to automate Sales Ops, SEO, and Marketing.`;
